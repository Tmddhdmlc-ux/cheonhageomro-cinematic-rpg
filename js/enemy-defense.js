(function(W){
  ...
  class EnemyDefenseController{
    ...
    start(target,attacker,snapshot){
      ...
      const from=target.currentPose(),originX=target.x,originY=target.y,offset=type==="sidestep"?-target.side*(snapshot.attackType==="SLASH"?72:60):0;
      this.active={target,attacker,snapshot,type,meta,from,to:POSES[type],originX,originY,offset,t:0};
      if(type==="sidestep"){
        ...
        target.setPose(POSES[type]);const a=attacker.getSwordTip(),b=target.getSwordTip(),point={x:(a.x+b.x)/2,y:(a.y+b.y)/2};
